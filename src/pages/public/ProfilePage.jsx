...
import { ArrowLeft, BookOpen, ChevronDown, ChevronUp, Pencil, X, Check, ThumbsUp, Lightbulb } from 'lucide-react'
...
function DueDateBadge({ borrowing }) {
  ...
  if (borrowing.status === 'en_retard') {
    ...
    return <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">En retard de {late} jour{late > 1 ? 's' : ''}</span>
  }
...
// ── Onglets ───────────────────────────────────────────────────
const TABS = ['Emprunts', 'Historique', 'Suggestions']
...
function ProfilePage() {
  ...
  const handleVote = async (suggestion) => {
    ...
    if (hasVoted) {
      await supabase.from('suggestion_votes').delete()
        .eq('suggestion_id', suggestion.id).eq('member_id', userId)
  ...
  const handleSugSubmit = async (e) => {
    ...
    const { error } = await supabase.from('suggestions').insert([{
      title:     sugForm.title.trim(),
      author:    sugForm.author.trim() || null,
      member_id: userId,
    }])

    if (error) { setSugError("Erreur lors de l'envoi. Réessayez.") }
    else {
      setSugSuccess('Suggestion envoyée ! Les autres membres peuvent voter pour votre proposition.')
      setSugForm({ title: '', author: '' })
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-100 sticky top-0 z-10">
        <div className="max-w-2xl mx-auto px-4 h-14 flex items-center justify-between">
          <button onClick={() => navigate('/')} className="flex items-center gap-1.5 text-gray-500 hover:text-gray-700 transition-colors">
            <ArrowLeft className="w-4 h-4" /><span className="text-sm">Catalogue</span>
          </button>
        ...
        <div className="flex gap-1 bg-white rounded-2xl border border-gray-100 shadow-sm p-1">
          {TABS.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              ...
              {tab === 'Suggestions' ? <span className="flex items-center justify-center gap-1"><Lightbulb className="w-3 h-3" />{tab}</span> : tab}
            </button>
          ))}
        </div>
              ...
              {history.map(b => (
                <div key={b.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-3 flex items-center gap-3">
                  <div className="w-10 h-14 bg-gray-100 rounded-lg overflow-hidden flex-shrink-0">
                    {b.books?.cover_url ? <img src={b.books.cover_url} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-200" />}
                  </div>
                  <div className="flex-1 min-w-0">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="text-xs font-semibold text-gray-600">{new Date(b.returned_at).toLocaleDateString('fr-FR')}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
              ...
              <form onSubmit={handleSugSubmit} className="space-y-3">
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Titre du livre *</label>
                  <input type="text" value={sugForm.title} onChange={e => setSugForm(p => ({ ...p, title: e.target.value }))}
                    placeholder="Titre du livre à ajouter"
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-600 mb-1">Auteur</label>
                  <input type="text" value={sugForm.author} onChange={e => setSugForm(p => ({ ...p, author: e.target.value }))}
                    placeholder="Nom de l'auteur (optionnel)"
                    className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-green-500" />
                </div>
                <button type="submit" disabled={savingSug}
                  className="w-full bg-green-700 text-white py-2.5 rounded-xl text-sm font-semibold hover:bg-green-800 disabled:opacity-50 transition-colors">
            ...
            {suggestions.length === 0 ? (
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-8 text-center text-gray-400">
                <p className="text-sm">Aucune suggestion pour l'instant. Soyez le premier !</p>
              </div>
            ) : (
              <div className="space-y-2">
                {suggestions.map(sug => {
                  ...
                  return (
                    <div key={sug.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        {sug.status !== 'pending' && (
                          <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${sug.status === 'purchased' ? 'bg-green-100 text-green-700' : 'bg-red-50 text-red-500'}`}>
                            {sug.status === 'purchased' ? 'Acheté ✓' : 'Refusé'}
                          </span>
                        )}

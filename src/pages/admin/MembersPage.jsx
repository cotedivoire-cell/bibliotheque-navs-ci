...
function MemberDrawer({ member, isOpen, onClose, onUpdate }) {
  ...
  const loadGroupData = async () => {
    ...
    const [borrowsRes, booksRes] = await Promise.all([
      supabase.from('borrowings')
        ...
        .eq('member_id', member.id).in('status', ['en_cours', 'en_retard'])
  ...
  const handleAttributeBook = async () => {
    ...
    if (!book) return
    setAttributing(true)
    const { data: dur } = await supabase.from('settings').select('value').eq('key', 'standard_borrow_duration').single()
    const days = parseInt(dur?.value || '14')
    const dueDate = new Date(Date.now() + days * 24 * 60 * 60 * 1000).toISOString()
    const { error } = await supabase.from('borrowings').insert([{
      book_id: selectedBookId, member_id: member.id,
      loan_type: 'location', amount_paid: 0,
      borrowed_at: new Date().toISOString(), due_date: dueDate, status: 'en_cours',
    }])
    if (!error) {
      await supabase.from('books').update({ available_copies: Math.max(0, book.available_copies - 1) }).eq('id', selectedBookId)
      await loadGroupData()
    }
  ...
  const handleReturnBook = async (borrowing) => {
    ...
    await supabase.from('borrowings').update({ status: 'retourné', returned_at: new Date().toISOString() }).eq('id', borrowing.id)
    const { data: bk } = await supabase.from('books').select('available_copies, total_copies').eq('id', borrowing.book_id).single()
    if (bk) await supabase.from('books').update({ available_copies: Math.min(bk.total_copies, bk.available_copies + 1) }).eq('id', borrowing.book_id)
    await loadGroupData()
    setReturning(null)
  }
            ...
            <div className="space-y-4">
              <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <p className="text-sm font-bold text-slate-900">{groupBorrows.length} / {member.max_borrowings || 10}</p>
                </div>
                <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                  <div className="h-full rounded-full bg-violet-500 transition-all duration-500"
                    style={{ width: Math.min(100, Math.round(groupBorrows.length / (member.max_borrowings || 10) * 100)) + '%' }} />
                </div>
              </div>
              ...
              {groupBorrows.length >= (member.max_borrowings || 10) ? (
                <div className="flex items-start gap-2 bg-red-50 border border-red-100 rounded-xl p-3">
                  ...
                  <p className="text-xs text-red-600">Quota maximum de {member.max_borrowings || 10} livres atteint.</p>
                </div>
              ) : (
                  ...
                  <div className="relative">
                    ...
                    {showDrop && !selectedBookId && (
                      <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-10 max-h-48 overflow-y-auto">
                        {availableBooks.filter(b => !bookSearch || b.title.toLowerCase().includes(bookSearch.toLowerCase()) || b.author.toLowerCase().includes(bookSearch.toLowerCase())).slice(0, 8).map((book, i, arr) => (
                          <button key={book.id} type="button"
                            ...
                            <span className="flex-shrink-0 bg-green-50 text-green-700 text-xs px-2 py-0.5 rounded-full font-medium">{book.available_copies} dispo.</span>
                          </button>
                        ))}
                      </div>
                    )}
                  </div>
                  <button onClick={handleAttributeBook} disabled={!selectedBookId || attributing}
              ...
              <div>
                ...
                {groupLoading ? (
                  <div className="space-y-2 animate-pulse">{[1,2].map(i => <div key={i} className="h-14 bg-slate-100 rounded-xl" />)}</div>
                ) : groupBorrows.length === 0 ? (
                  <div className="text-center py-6">
                    <BookOpen className="w-7 h-7 text-slate-200 mx-auto mb-2" strokeWidth={1} />
                    <p className="text-slate-400 text-xs font-light">Aucun livre attribué</p>
                  </div>
                ) : (
                  <div className="bg-white rounded-2xl border border-slate-100 overflow-hidden">
                    {groupBorrows.map((borrow, i) => (
                      <div key={borrow.id} className={"flex items-center gap-3 px-4 py-3 " + (i < groupBorrows.length - 1 ? "border-b border-slate-50" : "")}>
                        <div className="w-8 h-11 bg-slate-100 flex-shrink-0 overflow-hidden" style={{ borderRadius: 0 }}>
                          {borrow.books?.cover_url
                            ...
                            : <div className="w-full h-full bg-gradient-to-br from-green-900 to-green-700 flex items-center justify-center">
                                <span className="text-white text-[9px] font-bold">{borrow.books?.title?.charAt(0)}</span>
                              </div>
                          }
                        </div>
                        <div className="flex-1 min-w-0">
                        ...
                        <button onClick={() => handleReturnBook(borrow)} disabled={returning === borrow.id}
                          ...
                          {returning === borrow.id ? <span className="text-xs">...</span> : <RotateCcw className="w-4 h-4" strokeWidth={1.5} />}
                        </button>
                      </div>
                    ))}
                  </div>
      ...
      />
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between mt-4 pt-4 border-t border-slate-100">
            <p className="text-xs text-slate-400">{(page-1)*PAGE_SIZE+1}–{Math.min(page*PAGE_SIZE,filtered.length)} sur {filtered.length}</p>
            <div className="flex items-center gap-1">
              <button onClick={() => setPage(p => Math.max(1,p-1))} disabled={page===1} className="px-3 py-1.5 text-xs border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-30 transition-colors">←</button>
              {Array.from({length:totalPages},(_,i)=>i+1).map(n=>(
                <button key={n} onClick={()=>setPage(n)} className={"w-8 h-8 text-xs rounded-lg transition-colors "+(n===page?'bg-green-700 text-white font-semibold':'border border-slate-200 text-slate-500 hover:bg-slate-50')}>{n}</button>
              ))}
              <button onClick={() => setPage(p => Math.min(totalPages,p+1))} disabled={page===totalPages} className="px-3 py-1.5 text-xs border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-30 transition-colors">→</button>
            </div>
          </div>
        )}
    </AdminLayout>

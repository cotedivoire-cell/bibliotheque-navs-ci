...
/* ── Carte membre — clic sur la ligne principale ouvre le panneau ── */
function MemberCard({ member, onUpdate }) {
  const [open,      setOpen]      = useState(false)
  const [activeTab, setActiveTab] = useState('edit') // 'edit' | 'stats'
  ...
  /* Ouvre le panneau et charge les stats si besoin */
  ...
  const handleSaveEdit = async () => {
    ...
    setSaving(false)
    setOpen(false)
    onUpdate(member.id, { full_name: editForm.full_name.trim(), phone: editForm.phone.trim() })
  }
  ...
  return (
    <div className={`bg-white rounded-2xl shadow-sm border transition-all ${isBlocked ? 'border-red-100' : 'border-slate-100/50'}`}>
      ...
      {/* ── Ligne principale — clic pour ouvrir le panneau ── */}
      <div
        className="p-4 flex items-center gap-3 cursor-pointer select-none hover:bg-slate-50/50 rounded-t-2xl transition-colors"
        onClick={() => { setActiveTab('edit'); setOpen(v => !v) }}
      >
        {/* Avatar */}
        ...
        {/* Boutons rapides indépendants (stopPropagation) */}
        <div className="flex items-center gap-1 flex-shrink-0" onClick={e => e.stopPropagation()}>

          {isPending && (
            <button onClick={handleActivate}
              className="flex items-center gap-1 px-2.5 py-1.5 bg-green-700 text-white rounded-xl text-xs font-semibold hover:bg-green-800 transition-colors">
          ...
          <button onClick={handleToggleGroup}
            title={isGroup ? 'Passer en individuel' : 'Passer en groupe'}
            className={`p-1.5 rounded-xl transition-all ${isGroup ? 'text-violet-600 bg-violet-50 hover:bg-violet-100' : 'text-slate-300 hover:text-violet-600 hover:bg-violet-50'}`}>
            <Users className="w-4 h-4" strokeWidth={1.5} />
          </button>

          {/* Toggle bloquer */}
          <button onClick={handleToggleBlock}
            className={`relative w-9 h-5 rounded-full transition-colors flex-shrink-0 ${isBlocked ? 'bg-red-400' : 'bg-green-500'}`}>
            <div className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow-sm transition-all duration-200 ${isBlocked ? 'left-0.5' : 'left-4'}`} />
          </button>
        ...
        <div className="border-t border-slate-100">
          ...
          <div className="flex gap-1 p-3 pb-0">
            <button onClick={() => setActiveTab('edit')}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${activeTab === 'edit' ? 'bg-green-700 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>
              <Pencil className="w-3 h-3" strokeWidth={1.5} />Modifier
            </button>
            <button onClick={handleOpenStats}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium transition-all ${activeTab === 'stats' ? 'bg-blue-600 text-white' : 'text-slate-500 hover:bg-slate-100'}`}>
              <BarChart2 className="w-3 h-3" strokeWidth={1.5} />Statistiques
            </button>
          </div>

          {/* Contenu onglet Modifier */}
          {activeTab === 'edit' && (
            <div className="p-4 space-y-3">
              ...
              <div className="flex items-center gap-2 pt-1">
                <span className="text-xs text-slate-400 font-light flex-1">
                  Formule : <span className="font-medium text-slate-600">
                    {member.membership_type === 'annual' ? 'Abonnement annuel' : "À l'unité"}
                  </span>
                  {' · '}Max : <span className="font-medium text-slate-600">{member.max_borrowings || 3} emprunts</span>
                </span>
                ...
                <button onClick={handleSaveEdit} disabled={saving}
                  className="flex items-center gap-1.5 px-4 py-2 text-xs text-white bg-green-700 rounded-xl hover:bg-green-800 disabled:opacity-50 font-semibold transition-colors">
...
function MembersPage() {
  ...
  /* Met à jour un membre localement sans recharger toute la liste */
  ...
  const filtered = members.filter(m => {
    ...
    const matchFilter = !activeFilter
      || (activeFilter === 'unit'    && ['actif_unite','actif'].includes(m.profile_status) && m.membership_type === 'unit')
  ...
  const filterLabel = { unit:"À l'unité", annual:'Abonnement', group:'Groupes', pending:'En attente', blocked:'Bloqués' }
    ...
    <AdminLayout>
      ...
      {activeFilter && (
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs text-slate-500">Filtre : <span className="font-semibold text-green-700">{filterLabel[activeFilter]}</span></span>
          <button onClick={() => setActiveFilter(null)} className="text-xs text-slate-400 hover:text-rose-500 flex items-center gap-1">
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map(member => (
            <MemberCard key={member.id} member={member} onUpdate={handleUpdate} />
          ))}
        </div>
      )}
    </AdminLayout>

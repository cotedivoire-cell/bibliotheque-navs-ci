...
// ── Initiales pour la couverture de remplacement ───────────────
...
function BookCard({ book, onClick }) {
  ...
  const categoryStyle = CATEGORY_COLORS[book.category] || 'bg-gray-100 text-gray-500'
    ...
    >
      ...
      <div className="aspect-[2/3] relative overflow-hidden bg-gray-100">
        ...
        <div className={`absolute top-2 right-2 flex items-center gap-1
          ...
          <span className={`w-1.5 h-1.5 rounded-full ${available ? 'bg-white' : 'bg-white'}`} />
      ...
      <div className="p-3">
        ...
        {book.category && (
          <span className={`inline-block mt-2 text-xs px-2 py-0.5 rounded-full font-medium ${categoryStyle}`}>
            {book.category}
          </span>
        )}
      </div>
